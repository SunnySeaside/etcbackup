...
def read_yaml_list(obj,name):
    ...
    if type(data) is list:
        ...
        sys.exit("Error: "+name+" must be a list of string")

#TODO:
#-p: print list to be dumped
#dryrun
#-c: create
...
try:
    ...
    sys.exit("Error: no configuration file found at "+config_path)
...
for reponame,repoopts in config["repos"].items():
    ...
    if repotype=="normal":
        ...
        #TODO exclude paths
        backend.backup_files(repodir,passphrase,opts,list(paths))
    elif repotype=="data":
        databackup=backend.BackupRawData(repodir,passphrase,opts)
        for mod in mods:
    ...
    backend.purge(repodir,passphrase,opts)
